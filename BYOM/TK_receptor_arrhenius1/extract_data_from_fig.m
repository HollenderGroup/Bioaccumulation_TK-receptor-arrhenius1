...
open('fit_byom_temp_thi_GF16_1.fig') ; % choose corresponding file name of MATLAB figure
...
ydata1= get(h(5,1),'YData'); % mean
ydata2= get(h(3,1),'YData'); % upper Limit
ydata3= get(h(4,1),'YData'); % lower limit

% % Internal conentrations in receptor-complex/membrane (in µmol/kg) 
% NOTE: Nominal exposure concentrations are marking the lines
ydata4= get(h(8,1),'YData'); % mean
ydata5= get(h(6,1),'YData'); % upper Limit
ydata6= get(h(7,1),'YData'); % lower limit

% % Internal conentrations supertenant+debris (in µmol/kg) 
% NOTE: Nominal exposure concentrations are marking the lines
ydata7= get(h(11,1),'YData'); % mean
ydata8= get(h(9,1),'YData'); % upper Limit
ydata9= get(h(10,1),'YData'); % lower limit
...
dlmwrite('fit_byom_temp_thi_GF16_1.txt', fig01, ','); % write dataframe in txt
